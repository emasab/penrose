	...
	this.draw = function(graphics, penrose){
		...
		if(vs1 && vs1.length>0){
			...
			for(var i=0;i<vs1.length;i++){
				var vs2 = vs1[i];
				if(i==0) graphics.moveTo(vs2[X],vs2[Y]);
				else graphics.lineTo(vs2[X],vs2[Y]);
			}
			graphics.endFill();
		}
		var contourType = this.conf.contours[this.type];
		for(var i=0;i<vertices.length;i++){
			var vertex = vertices[i];
			var next = vertices[(i+1) % vertices.length];
			var vs1 = intersection([vertex,next], clipping);
			if(vs1 && vs1.length>0){
				...
				graphics.moveTo(vs1[0][X],vs1[0][Y]);
				graphics.lineTo(vs1[1][X],vs1[1][Y]);
				graphics.endFill();
			}
		}
	}
...
var Penrose = function(canvas, conf){
	...
	this.clone = function(conf){
	    ...
	    this.canvas.removeChild(this.renderer.view);
	    this.dismissed = true;
	    return penrose1;
	}

	this.scaleShapes = function(lastWidth){
		  var ratio = this.width / lastWidth;
	...
	this.resizeCanvas = function(width){
		...
		if(!width){
			this.width = parseInt(document.body.offsetWidth * 0.6);
		} else this.width = width;
		this.height = this.width / screen.width * screen.height;
		if(this.shapes){
	...
	this.fullscreen = function(){
		...
		} else if (this.view.msRequestFullscreen) {
		  this.view.msRequestFullscreen();
	...
	var s1 = conf.createFirstShape(this);
	var shapes = s1.subdivide();
	shapes = this.decompose(shapes, this.conf.initial_subdivisions);
